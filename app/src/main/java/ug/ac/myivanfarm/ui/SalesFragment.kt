...
import ug.ac.myivanfarm.databinding.FragmentSalesBinding
                    ...
                    val room = viewModel.getAllDbData(requireContext()).collect{
                        if(it != null){
                            withContext(Dispatchers.Main){
                                ...
                                val adapter = SaleAdapter(requireContext(), it)
...
//findNavController().navigate(DasboardFragmentDirections.actionDasboardNavToSaleAddFragment() )
        }
        return binding.root
    }
